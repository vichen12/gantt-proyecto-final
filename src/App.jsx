...
export default function App() {
  ...
  const updateTooltip = (task, e) => {
    ...
    if (el) {
      ...
      if (ty + height > window.innerHeight - 8) ty = e.clientY - height - 10
      if (tx + width > window.innerWidth - 8) tx = e.clientX - width - 16
    }

    setTooltip({ x: tx, y: ty, task, days, blockName: blockMap[task.block]?.name || task.block })
  }
